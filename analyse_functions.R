...
#####################################################
## Create settings to apply to all functions (if testing)
## Declare formula
...
run_dtree3 <- function(data,death_weight=1) {
  ## Use party package
  library(party)
  data_new <- data
  data_new[death=="no (0)",weight:=1]
  data_new[death=="yes (1)",weight:=death_weight]
...
## Random Forests
run_rf <- function(data,ntree,sample_weights) {
